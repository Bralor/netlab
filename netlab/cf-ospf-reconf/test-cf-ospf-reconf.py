...
def test_ospf_configure_undo(exp_devs: str):
    ...
    captured_stdout = tk.run_command(exp_devs, command="configure undo")
    assert "Undo requested" in captured_stdout[1] \
...
@pytest.mark.parametrize("exp_devs", EXPECTED_DEVICES)
def test_ospf_neighbors_ospf4_after_undo(exp_devs: str):
    """IPv4: After reconfiguration check the neighbor\'s tables"""
    tk.test_ospf_neighbors(key="neighbor-cf3", dev=exp_devs, protocol="ospf4")


@pytest.mark.parametrize("exp_devs", EXPECTED_DEVICES)
def test_ospf_neighbors_ospf5_after_undo(exp_devs: str):
    """IPv6: After reconfiguration check the neighbor\'s tables"""
    tk.test_ospf_neighbors(key="neighbor-cf3", dev=exp_devs, protocol="ospf5")


@pytest.mark.parametrize("exp_devs", EXPECTED_DEVICES)
def test_ospf_neighbors_ospf6_after_undo(exp_devs: str):
    """IPv6: After reconfiguration check the neighbor\'s tables"""
    tk.test_ospf_neighbors(key="neighbor-cf3", dev=exp_devs, protocol="ospf6")
